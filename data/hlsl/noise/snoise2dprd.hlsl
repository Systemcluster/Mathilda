...
//
// Description:
// 		Periodic Simplex Noise 2D

// 		float3 psrdnoise2d(float2 pos, float2 per, float rot)
// 		float3 psdnoise2d(float2 pos, float2 per)
// 		float  psrnoise2d(float2 pos, float2 per, float rot)
// 		float  psnoise2d(float2 pos, float2 per)
// 		float3 srdnoise2d(float2 pos, float rot)
// 		float3 sdnoise2d(float2 pos)
// 		float  srnoise2d(float2 pos, float rot)
// 		float  snoise2d(float2 pos)
...
// 		float3 {p}s{r}dnoise2d(float2 pos {, float2 per} {, float rot})
...
// 		float {p}s{r}noise2d(float2 pos {, float2 per} {, float rot})
// 		"pos" is the input (x,y) coordinate
// 		"per" is the x and y period, where per.x is a positive integer
// 		and per.y is a positive even integer
// 		"rot" is the angle to rotate the gradients (any float value,
...
// 		Author:
// 		Stefan Gustavson (stefan.gustavson@gmail.com) - Original WebGL Version
...
// 		Copyright (c) 2016 Stefan Gustavson

// 		Original GLSL version:  https://github.com/stegu/webgl-noise

// License: 
//		MIT

// Notes:
// 		One-pixel wide artefacts used to occur due to precision issues with
...
float2 rgrad2(float2 p, float rot) {
		...
		float u = permute(permute(p.x) + p.y) * (1.0/41.0) + rot; // Rotate by shift
		u = 4.0 * frac(u) - 2.0;
...
//
float3 psdnoise2d(float2 pos, float2 per) {
	return psrdnoise2d(pos, per, 0.0);
}
