...
//
// Description : Array and textureless 4D simplex
//               noise function.
...
float snoise4d(float4 v, out float4 gradient) {
	const float4 C = float4(0.138196601125011, // (5 - sqrt(5))/20  G4
		0.276393202250021, // 2 * G4
		0.414589803375032, // 3 * G4
		-0.447213595499958); // -1 + 4 * G4
	...
	return 109.319 * (dot(m02 * m02, float3(dot(p0, x0), dot(p1, x1), dot(p2, x2))) + dot(m12 * m12, float2(dot(p3, x3), dot(p4, x4))));
}
