...
// after a quick fix was introduced: a small offset is added to the y
// coordinate. A proper fix would involve using round() instead of floor() in
// selected places, but the quick fix works fine. (If you run into problems with
...
//
// 2-D non-tiling simplex noise with rotating gradients and analytical
// derivative. The first component of the 3-element return vector is the noise
// value, and the second and third components are the x and y partial
// derivatives.
//
vec3 srdnoise(vec2 pos, float rot) {
	// Offset y slightly to hide some rare artifacts
